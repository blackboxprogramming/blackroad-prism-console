...
import { toVtt } from '../src/pipeline/format/vtt';
...
describe('toVtt', () => {
  it('matches the golden snapshot for the sample audio', () => {
    const actual = toVtt(SAMPLE_CHUNKS);
    const expected = fs.readFileSync(path.join(__dirname, 'snapshots', 'sample_audio.vtt.golden'), 'utf8');
