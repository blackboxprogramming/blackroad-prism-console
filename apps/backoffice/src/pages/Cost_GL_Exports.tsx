...
export default function Cost_GL_Exports(){
  const [period,setPeriod]=useState(new Date().toISOString().slice(0,7)); const [items,setItems]=useState<any[]>([]);
  const run=async()=>{ await fetch('/api/cost/gl/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({period})}); await load(); };
  const load=async()=>{ const j=await (await fetch('/api/cost/gl/recent')).json(); setItems(j.items||[]); };
  useEffect(()=>{ load(); },[]);
  return <section><h2>GL Export (COGS/INV/WIP)</h2>
