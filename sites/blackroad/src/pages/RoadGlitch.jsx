...
export default function RoadGlitch(){
  ...
  const glitchText=(txt)=>txt.split('').map(ch=>Math.random()>0.8?String.fromCharCode(33+Math.random()*94):ch).join('')
  ...
  return (
    <div className="card relative overflow-hidden" style={{filter:`${filters[mood]} hue-rotate(${shift}deg)`}} onMouseMove={()=>setShift(s=>s+5)}>
      <style>{`
      ...
      <form onSubmit={runGlitch} className="space-y-2">
        <textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="Enter prompt..." className="w-full p-2 rounded bg-neutral-900 border border-neutral-700"/>
        <div className="flex gap-2 items-center">
          <select value={mood} onChange={e=>setMood(e.target.value)} className="bg-neutral-900 border border-neutral-700 rounded p-2">
            {moods.map(m=>(<option key={m}>{m}</option>))}
          </select>
          <button type="submit" className="btn-secondary">Glitch</button>
        </div>
      </form>
