...
function parseYAML(s) {
  ...
  for (const l of lines) {
    ...
    if (mode === 'tasks') {
      ...
      if (!cur) continue;
      if ((m = l.match(/^\s+prompt:\s*\|/))) {
        inBlock = true;
        bkey = 'prompt';
        buf = [];
      } else if ((m = l.match(/^\s+expect_keywords:\s*\[(.*)\]\s*$/))) {
        cur.expect_keywords = m[1]
          .split(',')
          .map((x) => x.trim())
          .filter(Boolean);
      } else if ((m = l.match(/^\s+meta:\s*\|/))) {
        inBlock = true;
        bkey = 'meta';
        buf = [];
      }
...
async function callOllama(prompt) {
  ...
  while (attempt < 3) {
    try {
      const r = await httpPostJSON(`${base.replace(/\/$/, '')}/api/generate`, {
        model,
        prompt,
        stream: false,
      });
      ...
      let resp = '';
      try {
        resp = JSON.parse(r.body).response || '';
      } catch {
        resp = r.body.slice(0, 2000);
      }
...
async function run() {
  if (!fs.existsSync(suitesDir)) {
    console.log('No prompts/llm; skipping.');
    return;
  }
  ...
  for (const f of files) {
    ...
    for (const t of suite) {
      ...
      results.push({
        ...
        prompt: t.prompt,
        response,
        expect_keywords: t.expect_keywords || [],
        latencyMs,
      });
      if (latencyMs) latency.push(latencyMs);
    }
    fs.writeFileSync(
      path.join(outDir, f.replace(/\.ya?ml$/, '') + '.json'),
      JSON.stringify({ suite: f, results }, null, 2)
    );
  }
  if (latency.length) {
    ...
    const summary = {
      ...
      p95: p(95),
      p99: p(99),
      avg: Math.round(latency.reduce((a, b) => a + b, 0) / latency.length),
    };
...
run().catch((e) => {
  console.log('eval error (non-fatal):', e.message);
  process.exit(0);
});
